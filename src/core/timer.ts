...
//
// 이 모듈은 DOM을 모른다. 화면 갱신은 ui/quiz.ts가 맡는다.
...
export interface QuestionTimerOptions {
  limitMs?: number;
  /** 기본이 performance.now다. Date.now와 달리 시스템 시계가 바뀌어도 영향받지 않는다. */
  now?: () => number;
}
...
export function createQuestionTimer({
  ...
  now = () => performance.now(),
}: QuestionTimerOptions = {}): QuestionTimer {
