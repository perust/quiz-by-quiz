...
//
// 랭킹(`storage/local-store.ts`)과 온라인 방(`online/local-rooms.ts`)이 둘 다
 ...
 * 이 프로젝트가 저장소에게 바라는 것.
 *
 * `Storage`(localStorage의 타입) 전체가 아니라 실제로 쓰는 세 가지만 적는다.
 * 그래야 메모리 대체품이 `length`나 `key()`까지 흉내 내지 않아도 된다.
 */
...
function resolveStorage(): KeyValueStorage {
  try {
    ...
    // 저장소가 던지는 것은 DOMException 이라 message 가 있다. 알림 문구를 만드는 자리라
    // 좁히지 않고 그대로 읽는다 — 원래 코드와 같은 말이 찍혀야 한다
    console.warn(`[저장소] localStorage를 쓸 수 없어 이번 세션에만 기록을 유지합니다: ${(error as Error).message}`);
 ...
 */
export const isPersistent: boolean = safeStorage === window.localStorage;
