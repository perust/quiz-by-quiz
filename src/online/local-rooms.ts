...
// (`adapter.ts` 한 줄) — 랭킹 저장소와 같은 방식이다.
//
// **localStorage를 직접 만지지 않고 `storage/safe-storage.ts`를 거친다.**
...
import type {
  ...
  PlayerInfo, StartGameResult, Unsubscribe,
} from './adapter.js';
 ...
 * 방 안에 저장해 두는 참가자.
 *
 * **`seenAt`이 없을 수 있다** — 신분(`quiz.playerId`)을 남기기 전에 만들어진 자취라
 * 주인이 다시 찾아올 수 없다. prune이 그런 참가자를 떨군다.
 */
 ...
 * 저장소 안에 있는 그대로의 방.
 *
 * **이 모습은 이 파일 밖으로 나가지 않는다.** 비밀번호와 마지막으로 본 시각이
 * 여기 있고, 밖으로는 `toPublic`이 만든 `PublicRoom`만 나간다.
 */
interface Room {
  code: string;
  name: string;
...
function readAll(): Room[] {
  try {
    ...
    // **«성하다»의 기준은 code 하나뿐이다** — 나머지가 비어 있어도 prune·toPublic 이 감당한다
 ...
 */
function writeAll(rooms: Room[]): boolean {
  try {
 ...
 *
 * 돌려주는 타입에 비밀번호 자리가 없어 **실수로 실어 보낼 수도 없다.**
 */
...
export const localRooms = {
  ...
  async myRooms(): Promise<PublicRoom[]> {
   ...
   */
  async createRoom({ name, categoryId, capacity, isPublic, password, player }: CreateRoomSpec): Promise<CreateRoomResult> {
   ...
   */
  async joinRoom({ code, password, player }: JoinRoomSpec): Promise<JoinRoomResult> {
  ...
  /** 방 하나를 읽는다. 없으면 null */
  async getRoom(code: string): Promise<PublicRoom | null> {
    const rooms = readAll();
   ...
   *
   * 이벤트는 `{type: 'chat' | 'room'}` 등. 서버 구현에서는 여기가 소켓이 된다.
   */
   ...
   */
  async updateRoom({ code, patch }: { code: string; patch: RoomPatch }): Promise<RoomActionResult> {
    const rooms = readAll();
    const room = rooms.find((item) => item.code === normalizeCode(code));
    if (!room) return { ok: false, reason: 'not-found' };
   ...
   * 방에 있는 모두에게 간다.
   */
  async sendChat({ code, text, player }: { code: string; text: string; player?: PlayerInfo }): Promise<{ ok: boolean }> {
   ...
   */
  async startGame({ code }: { code: string }): Promise<StartGameResult> {
    const rooms = readAll();
    const room = rooms.find((item) => item.code === normalizeCode(code));
    if (!room) return { ok: false, reason: 'not-found' };
    if (room.hostId !== meId) return { ok: false, reason: 'not-host' };
