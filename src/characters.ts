...
// box-shadow로 펼쳐 그린다 (ui/sprite.ts). 이 프로젝트는 바이너리 자산을 두지 않는다.
//
// 도트 그림은 문자 하나가 픽셀 하나다. 공백(또는 '.')은 비운다.
// 글자는 palette의 키이며, 어느 글자를 쓰든 상관없다.
//
// **name은 화면에 그리지 않는다.** 무엇으로 보이는지는 보는 사람이 정한다.
// 다만 스크린리더에는 이 값을 알려준다 — 그림만 있는 버튼은 눈으로 볼 수 없는
// 사람에게 아무것도 아니고, «캐릭터 3» 같은 자리 번호보다 실제 이름이 쓸모 있다.
// 눈으로 보는 사람에게만 이름을 감추는 것이 목적이다.
