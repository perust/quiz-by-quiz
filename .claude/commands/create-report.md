...
**이 표가 등급 규칙의 정의다.** `/teacher-dashboard` 와 `/quiz-teacher` 도 같은 규칙으로
종합 등급을 표시한다. `/teacher-dashboard` 는 실행할 때마다 세 파일의 값이 일치하는지 스스로 검사한다. **값을 바꾸면 두 파일을 함께 고쳐야 한다.** 한쪽만 고치면
같은 학생이 대시보드와 성적표에서 다른 등급을 받는다.
...
- **등급이 낮은 이유를 문항 단위로 짚으려면** `/teacher-dashboard` 9구획을 본다.
  앱이 문항별 정오를 저장하므로 문항 정답률을 낼 수 있다.
  단, 앱 개선 이전에 푼 기록에는 문항 정오가 없어 그 건은 제외된다
