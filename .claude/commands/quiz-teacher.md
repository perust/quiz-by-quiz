...
> **`/teacher-dashboard` 가 이 명령어를 포함한다.** 그쪽은 여기의 네 단계에 더해
> 등급 분포·성취 히스토그램·히트맵·교육 제안까지 낸다. 처음 여는 화면으로는 그쪽을 권한다.
> 이 명령어는 **비교와 순위까지만 빠르게 보고 싶을 때** 쓴다.
...
- **문항별 분석은 이 명령어가 하지 않는다.** 앱이 문항별 정오를 저장하므로 분석 자체는 가능하다.
  `/teacher-dashboard` 9구획에서 문항 정답률·난이도 불일치·시간 초과를 본다.
