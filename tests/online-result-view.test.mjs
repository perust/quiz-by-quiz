...
test('online final result는 server score array 순서를 그대로 순위로 표시한다', async () => {
  ...
  const snapshot = {
    ...
    state: 'finished', categoryId: 'history', gameMode: false,
    ...
    scores: [
      { playerId: OTHER, nickname: '먼저 온 서버 순위', characterId: null, score: 0, correctCount: 0, answeredCount: 10 },
...
test('online final result는 current player가 score list에 없으면 존재하지 않는 순위를 만들지 않는다', async () => {
  ...
  const snapshot = {
    matchId: '32345678-1234-5678-9234-567812345678',
    state: 'finished', categoryId: null, gameMode: false,
    currentPosition: 10, totalQuestions: 10, deadlineAt: '2026-09-18T11:00:20+00:00',
    question: null, ownSubmission: null, reveal: null,
    scores: [{ playerId: OTHER, nickname: '다른 사람', characterId: null, score: 20, correctCount: 2, answeredCount: 10 }],
  };
