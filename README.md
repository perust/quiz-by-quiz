...
- 🙂 **내 캐릭터** — 슬라임 여섯 색과 도트 몬스터 여섯 마리 중에 고른다. **이름은 화면에 띄우지 않는다.**
  홈에서 캐릭터를 걸어 메뉴를 고를 수 있고, 그 자체가 게임 모드 조작 연습이 된다
- 🕹️ **게임 모드** — 십자로 나뉜 2×2 바닥 위를 캐릭터가 자유롭게 돌아다니고,
  **시간이 다 되면 그때 밟고 있는 칸이 답**이 된다. 밟은 칸에는 불이 들어온다.
  한가운데(중립)에서 시작하므로 움직이지 않으면 보통 모드와 똑같이 시간 초과다.
  PC는 방향키, 휴대폰은 화면에 뜨는 이동 스틱으로 움직인다. 언제든 켜고 끌 수 있다
...
js/characters.js      고를 수 있는 캐릭터 목록 (슬라임 색·도트 그림)
...
| --- | --- |
| `quiz.rankings` | 랭킹 기록 |
| `quiz.settings` | 음소거·게임 모드·고른 캐릭터 |
