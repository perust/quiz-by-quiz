...
- 닉네임을 넣어 도전 종류·카테고리별 상위 10개 기록을 이 브라우저에 저장한다.
- 🙂 **내 캐릭터** — 슬라임 여섯 색과 도트 몬스터 여섯 마리 중 하나를 고른다.
- 🕹️ **캐릭터 퀴즈** — 모든 플레이어가 십자로 나뉜 2×2 바닥 위에서 캐릭터를 움직여 보기를 선택한다. 별도 모드 선택은 없다.
...
돌려준 REST snapshot을 표시한다. WebSocket은 다시 읽어야 한다는 invalidation 신호와
인증된 참가자의 실시간 이동·채팅만 전달하며, 브라우저가 정답·점수·시간 초과를 판정하지
않는다. 이동·채팅의 발신자 신원은 client payload가 아니라 WebSocket ticket에서 정한다.
